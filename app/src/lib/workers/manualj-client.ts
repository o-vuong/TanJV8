...
import type { WorkerMessage, WorkerResponse } from '../../workers/manualj.worker'
...
export class ManualJWorkerClient {
  ...
  constructor() {
    this.worker = new Worker(new URL('../../workers/manualj.worker.ts', import.meta.url), {
      type: 'module',
    })
  ...
  calculate(
    ...
    return new Promise<CalculationResults>((resolve, reject) => {
      ...
      this.worker.postMessage({ id, type: 'CALCULATE', inputs } satisfies WorkerMessage)
    })
  }
