...
interface CrudFactoryOptions<TData, TCreate, TUpdate> {
  resource: string
  client?: ReturnType<typeof createApiClient>
  queryKeyBase?: unknown[]
  mapListParams?: (params?: Record<string, unknown>) => Record<string, string | number | boolean | undefined>
  mapCreateData?: (data: TCreate) => unknown
  mapUpdateData?: (data: TUpdate) => unknown
}

export function createCRUDHooks<TData extends { id: string }, TCreate = Partial<TData>, TUpdate = Partial<TData>>(
  ...
  function useCreate(
    ...
    return useMutation<TData, Error, TCreate>({
      mutationFn: (payload) =>
        client.post<TData>(`${basePath}`, mapCreateData ? mapCreateData(payload) : payload),
      onSuccess: (data, ...args) => {
  ...
  function useUpdate(
    mutationOptions?: UseMutationOptions<TData, Error, { id: string; data: TUpdate }>,
  ) {
    const queryClient = useQueryClient()
    return useMutation<TData, Error, { id: string; data: TUpdate }>({
      mutationFn: ({ id, data }) =>
        client.patch<TData>(`${basePath}/${id}`, mapUpdateData ? mapUpdateData(data) : data),
