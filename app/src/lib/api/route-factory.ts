...
>({
	...
	return {
		GET: async (request: Request) => {
			...
			const parsedQuery = schema?.query
				? schema.query.parse(rawQuery)
				: (rawQuery as TQuery);
		...
		POST: async (request: Request) => {
			...
			const parsed = schema?.create
				? schema.create.parse(json)
				: (json as TCreate);
			const args = buildCreateArgs
				? buildCreateArgs(parsed, request)
				: { data: parsed };
...
export function createEntityHandlers<T, TUpdate = Partial<T>>({
	...
	return {
		...
		PATCH: async (request: Request) => {
			...
			const params = await parseParams(request, schema?.params);
			const json = await request.json();
			const parsed = schema?.update
				? schema.update.parse(json)
				: (json as TUpdate);
		...
		DELETE: async (request: Request) => {
			...
			const args = buildDeleteArgs
				? buildDeleteArgs(params, request)
				: { where: { id: params.id } };
